...
def getTemp(month,monthNo):
	...
	tempQuery = "SELECT Value,month,Year FROM tempurature_past where month='"+month+"' and District = 'Badulla' and Year BETWEEN '2010' AND '2014' order by FIELD(MONTH,'January','February','March','April','May','June','July','August','September','October','November','December')"
...
tempQuery = "SELECT Value,month,Year FROM tempurature_past where District = 'Badulla' and Year = '2014' order by FIELD(MONTH,'January','February','March','April','May','June','July','August','September','October','November','December')"
tempData = dbQuery(tempQuery)
tempData = np.array(tempData) 
tempData = pd.DataFrame(tempData)
Yy = tempData.ix[:,0]
