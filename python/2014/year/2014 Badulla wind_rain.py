...
def getTemp(month,monthNo):
	query = "SELECT Value,month,Year FROM rainfall_past where month='"+month+"' and District = 'Badulla' and Year BETWEEN '2010' AND '2013' order by FIELD(MONTH,'January','February','March','April','May','June','July','August','September','October','November','December')"
	data = dbQuery(query)
	data = np.array(data) 
	data = pd.DataFrame(data) 
	months = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
	
	tempQuery = "SELECT Value,month,Year FROM wind_past where month='"+month+"' and District = 'Badulla' and Year BETWEEN '2010' AND '2014' order by FIELD(MONTH,'January','February','March','April','May','June','July','August','September','October','November','December')"
	...
	return regr.predict([(14,monthNo,7)])
...
tempQuery = "SELECT Value,month,Year FROM wind_past where District = 'Badulla' and Year = '2014' order by FIELD(MONTH,'January','February','March','April','May','June','July','August','September','October','November','December')"
tempData = dbQuery(tempQuery)
tempData = np.array(tempData) 
tempData = pd.DataFrame(tempData)
Yy = tempData.ix[:,0]
