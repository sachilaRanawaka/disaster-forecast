...
def getTemp(month,monthNo):
	query = "SELECT Value,month,Year FROM rainfall_past where month='"+month+"' and District = 'Bandarawela ' and Year BETWEEN '2010' AND '2013' order by FIELD(MONTH,'January','February','March','April','May','June','July','August','September','October','November','December')"
	data = dbQuery(query)
	data = np.array(data) 
	data = pd.DataFrame(data) 
	months = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
	
	tempQuery = "SELECT Value,month,Year FROM tempurature_past where month='"+month+"' and District = 'Bandarawela ' and Year BETWEEN '2010' AND '2014' order by FIELD(MONTH,'January','February','March','April','May','June','July','August','September','October','November','December')"
...
tempQuery = "SELECT Value,month,Year FROM tempurature_past where District = 'Bandarawela ' and Year = '2014' order by FIELD(MONTH,'January','February','March','April','May','June','July','August','September','October','November','December')"
tempData = dbQuery(tempQuery)
tempData = np.array(tempData) 
tempData = pd.DataFrame(tempData)
Yy = tempData.ix[:,0]
...
query = "SELECT Value,month FROM rainfall_past where District = 'Bandarawela ' and Year = '2014' order by FIELD(MONTH,'January','February','March','April','May','June','July','August','September','October','November','December')"
