...
def getTemp(month,monthNo):
	query = "SELECT Value,month,Year FROM rainfall_past where month='"+month+"' and District = 'Galle' and Year BETWEEN '2010' AND '2013'"
	data = dbQuery(query)
	data = np.array(data) 
	data = pd.DataFrame(data) 
	months = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
	
	tempQuery = "SELECT Value,month,Year FROM tempurature_past where month='"+month+"' and District = 'Galle' and Year BETWEEN '2010' AND '2013'"
	...
	return regr.predict([(14,monthNo,31)])
