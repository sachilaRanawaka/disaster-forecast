...
def getTemp(month,monthNo):
	query = "SELECT Value,month,Year FROM flood_forcast where month='"+month+"' and District = 'Anuradhapura' and Year BETWEEN '2006' AND '2015' order by FIELD(MONTH,'January','February','March','April','May','June','July','August','September','October','November','December')"
	data = dbQuery(query)
	data = np.array(data) 
